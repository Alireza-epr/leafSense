...
export const useMap = () => {
  ...
  const addMarker = async (
    ...
    const mapCallback = (maplibre) => {
      ...
      if (mapMarkers.has(a_Type) && a_Type !== "point") {
        ...
        else mapMarkers.set(a_Type, [markerElement]);
      } else {
    ...
    const marker = await import("maplibre-gl").then((maplibre) =>
      mapCallback(maplibre),
    );
  ...
  const addPointMarker = useCallback(
    async (a_Event: maplibregl.MapMouseEvent) => {
  ...
  const addPolygonMarker = async (a_Event: maplibregl.MapMouseEvent) => {
    if (a_Event.originalEvent.button === 2 && map) {
      ...
      const markersToAdd = mapMarkers.get(
        `${EMarkerType.polygon}-${lastZonalLayerId + 1}`,
      );
      if (markersToAdd && markersToAdd.length >= 3) {
        setPolygons((prev) => {
          return [
            ...prev,
            {
              id: `${EMarkerType.polygon}-${lastZonalLayerId + 1}`,
  ...
  const getAllZonalLayer = () => {
    ...
    return map
      .getLayersOrder()
      .filter(
        (l) => l.includes(`${EMarkerType.polygon}`) && !l.includes("label"),
      );
  };
