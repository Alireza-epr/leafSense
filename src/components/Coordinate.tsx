...
export interface ICoordinateProps {
  ...
  id: number,
  disabled?: boolean
  onCoordinateChange: (id: number, coordinate: ECoordinate, value: string) => void
}

const Coordinate = (props: ICoordinateProps) => {

  const onCoordinateChange = (a_Value: React.ChangeEvent<HTMLInputElement>, a_Coordinate: ECoordinate) => {
    if(!props.disabled){
        props.onCoordinateChange(
          props.id,
          a_Coordinate,
          a_Value.target.value
        )
    }
  }
  ...
  return (
    <div className={` ${coordinateStyles.wrapper}`}>
      <div className={` ${coordinateStyles.counter}`}>
        { props.id }
      </div>
